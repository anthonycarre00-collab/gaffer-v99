...
fn row_to_player(row: &rusqlite::Row) -> rusqlite::Result<Player> {
    ...
    Ok(Player {
        ...
        // V100 P0-8 (Issue #5): not_for_sale — not persisted to DB yet, default false.
        not_for_sale: false,
        // V100 P1 (Issue #3): Position retraining fields — not persisted to DB yet.
        training_position_focus: None,
        retraining_xp: 0,
        // V100 P1 (Issue #1): height/weight — not persisted to DB yet, will be
        // generated by refresh_player_derived on first load.
        height_cm: 0,
        weight_kg: 0,
    })
}
