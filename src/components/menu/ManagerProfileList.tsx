        ...
        <div className="flex flex-col gap-2 max-h-[13.5rem] overflow-y-auto pr-0.5">
            {profiles.map((profile) => {
                ...
                return (
                    <div
                        key={profile.id}
                        className={`group relative flex items-center w-full rounded-lg border transition-all duration-200 ${isSelected
                    ...
                    >
                        ...
                        {confirmDeleteId === profile.id && (
                            <div className={`absolute inset-0 flex items-center justify-end gap-1.5 px-3 rounded-lg ${isSelected ? "bg-primary-50 dark:bg-primary-500/10" : "bg-white dark:bg-navy-700"}`}>
                                <button
                                    type="button"
                                ...
                                <button
                                    ...
                                    className="px-2.5 py-1 bg-gray-200 hover:bg-gray-300 dark:bg-navy-800 dark:hover:bg-navy-900 text-gray-700 dark:text-gray-300 text-xs font-heading font-bold uppercase tracking-wider rounded transition-colors"
                                >
                                    {t("menu.cancel")}
                                </button>
                            </div>
